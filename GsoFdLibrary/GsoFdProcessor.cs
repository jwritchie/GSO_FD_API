...
namespace GsoFdLibrary
{
    public class GsoFdProcessor
    {
        ...
        public static async Task<string> LoadGsoFdData(Dictionary<string,string> queryData, bool limit, int totalRequested)
        {
            ...
            urlBuilder.Append($"&outFields=*");

            if (limit)
            {
                urlBuilder.Append($"&resultOffset=0&resultRecordCount={totalRequested}");
            }

            urlBuilder.Append($"&outSR=4326&f=json");
